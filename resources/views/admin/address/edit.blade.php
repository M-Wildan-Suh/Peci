<div x-show="edit" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div
        ...
        <form id="{{$item->id}}" action="{{ route('address.update', ['address' => $item->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-3 px-6 overflow-auto">
                <x-admin.component.textinput title="Nama Penerima" placeholder="Input Your Name..." :value="$item->name"
                    name="name" />
                <x-admin.component.linkinput title="No Telepone" placeholder="Input Phone Number..." :value="$item->phone_number"
                    name="phone_number" link="+62" />
                <x-admin.component.textareainput title="Address" placeholder="Input Address..." :value="$item->address"
                    name="address" />
                <x-admin.component.textareainput title="Aditional" placeholder="Input Aditional..." :value="$item->additional"
                    name="additional" />
            </div>
        </form>
